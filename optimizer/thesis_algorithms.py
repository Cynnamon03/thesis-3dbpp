...
class WolfContinuous:
    def __init__(self, n, lambda_w=1000.0, lambda_f=1000.0, lambda_b=1000.0, lambda_a=1000.0):
        self.n = n
    ...
    def decode_and_evaluate(self, items, container, apply_repair=False):
        ...
        while unpacked:
            ...
            if len(overflow) == len(unpacked):
                # No progress made (e.g. oversized items), prevent infinite loop
                break
        ...
        v_w = detail['C3_weight_v']
        v_f = detail['C4_fragility_v']
        v_b = detail['C5_balance_v']
        v_a = detail['C6_stop_order_v']
        
        penalty = (self.lambda_w * v_w) + (self.lambda_f * v_f) + (self.lambda_b * v_b) + (self.lambda_a * v_a)
        
        # Maximize F(X) = U(X) - penalty
    ...
    def _repair(self, items, container):
        ...
        from geometry_3d import can_place, _update_eps, get_dims
        ...
        for r_pass in range(R_max):
            ...
            for b, members in per_bin.items():
                # Get placed info for current bin
                bin_placed = [(p[1], p[2], p[3], p[4], p[5], p[6]) for j, p in self.placements.items() if p[0] == b]
                for i in members:
                    if i in violating_items: continue
                    boxes_above = [j for j in members if i != j and j not in violating_items and _is_above(i, j, self.placements)]
                    
                    # R1 (Fragility): Remove items stacked above fragile boxes and re-attempt
                    if items[i].get('fragile', 0) == 1 and len(boxes_above) > 0:
                        for upper in boxes_above:
                            violating_items.add(upper)
                            
                    # R2 (Weight): Remove items if cumulative mass > LBS
                    boxes_above_r2 = [j for j in boxes_above if j not in violating_items]
                    cumulative_weight = sum(items[j].get('Weight', items[j].get('weight', 1)) for j in boxes_above_r2)
                    if cumulative_weight > items[i].get('LBS', float('inf')):
                        boxes_above_r2.sort(key=lambda j: items[j].get('Weight', items[j].get('weight', 1)), reverse=True)
                        removed_weight = 0
                        for j in boxes_above_r2:
                    ...
                    if i not in violating_items:
                        (_, x_i, y_i, z_i, l_i, h_i, d_i) = self.placements[i]
                        if y_i > 0:
                            supported_area = 0.0
                            for j in members:
                                if i != j and j not in violating_items:
                    ...
                    if i not in violating_items:
                        ...
                        for j in members:
                            if i != j and j not in violating_items and _blocks_extraction(i, j, self.placements) and items[j].get('stop', 1) > s_i:
                                violating_items.add(j)
            
            if not violating_items:
                break
                
            # Perform re-placements and translations
            # Remove from placements first
            for v in violating_items:
                if v in self.placements:
                    del self.placements[v]
            
            # Extract discrete orientation mappings from the original continuous positions
            discrete_targets = decode_position(self.X, items, container)
            orients_map = {t['item_idx']: t['r'] for t in discrete_targets}
            
            # Re-attempt placement via DBLF (R1, R3 translations, R4 repositions)
            unpacked = list(violating_items)
            for v in unpacked:
                orient_id = orients_map.get(v, 0)
                l, h, d = get_dims(items[v], orient_id)
                placed_flag = False
                
                # Attempt to place in any existing bin
                for b in set(p[0] for p in self.placements.values()):
                    bin_placed = [(p[1], p[2], p[3], p[4], p[5], p[6]) for j, p in self.placements.items() if p[0] == b]
                    bin_eps = [(0,0,0)]
                    for p in bin_placed:
                        bin_eps = _update_eps(bin_eps, bin_placed, p[0], p[1], p[2], p[3], p[4], p[5], CL, CH, CD)
                    
                    for (ex, ey, ez) in bin_eps:
                        if can_place(ex, ey, ez, l, h, d, CL, CH, CD, bin_placed):
                            # R3 (Balance): Check 80% support threshold for re-placement
                            if ey > 0:
                                supported_area = 0.0
                                for p in bin_placed:
                                    if abs((p[1] + p[4]) - ey) < 1e-5:
                                        ox = _overlap(ex, ex + l, p[0], p[0] + p[3])
                                        oz = _overlap(ez, ez + d, p[2], p[2] + p[5])
                                        supported_area += ox * oz
                                base_area = l * d
                                if base_area > 0 and (supported_area / base_area) < 0.80:
                                    continue # Fails 80% support, try next EP
                            
                            self.placements[v] = (b, ex, ey, ez, l, h, d)
                            placed_flag = True
                            break
                    if placed_flag: break
                
                # If no valid position found, it remains deferred (unpacked)
                    
        # R5: Re-evaluate repaired arrangement
        self.su = space_utilization(self.placements, container, self.n_bins)
...
class ThesisOptimizerBase:
    def __init__(self, items, container, pop_size=30, max_iter=500, max_time=90, lambda_w=1000.0, lambda_f=1000.0, lambda_b=1000.0, lambda_a=1000.0, stream_cb=None, A_max=100):
        self.items = items
...
class StandaloneDGWO(ThesisOptimizerBase):
    def run(self):
        ...
        pop = [WolfContinuous(self.n, self.lambda_w, self.lambda_f, self.lambda_b, self.lambda_a) for _ in range(self.pop_size)]
        
        for w in pop:
            w.decode_and_evaluate(self.items, self.container)
            
        pop.sort(key=lambda w: w.scalar_fitness, reverse=True)
        alpha, beta, delta = pop[0], pop[1], pop[2]
        
        for iteration in range(self.max_iter):
            if time.time() - start_time > self.max_time:
                break
            a = 2.0 - 2.0 * (iteration + 1) / self.max_iter
...
class StandaloneMOGWO(ThesisOptimizerBase):
    ...
    def _update_archive(self, archive, wolf):
        ...
        if not is_dominated:
            ...
            w_copy = WolfContinuous(self.n, self.lambda_w, self.lambda_f, self.lambda_b, self.lambda_a)
...
class RepairBasedHybrid(StandaloneMOGWO):
    def run(self):
        ...
        for iteration in range(self.max_iter):
            ...
            # Limit archive size
            if len(archive) > self.A_max:
                archive = self._prune_archive(archive, max_size=self.A_max)
    ...
    def _select_leaders(self, archive):
        if len(archive) < 1:
            # Fallback if empty (shouldn't happen)
            fake = WolfContinuous(self.n, self.lambda_w, self.lambda_f, self.lambda_b, self.lambda_a)
            return fake, fake, fake
...
class SequentialHybrid(StandaloneMOGWO):
    def run(self, T1=None, T2=None):
        ...
        # Return best from archive based on U(X)
        archive.sort(key=lambda w: w.su, reverse=True)
        return archive[0]
